...
void Monster::displayStats() const {
    ...
    std::cout << getName() << " [HP: " 
              << getCurrentHP() << "/" << getMaxHP() << "]" << std::endl;
}
...
void Monster::addLoot(Item* item) {
    ...
    if (item) loot_table.push_back(item);
}
...
Skeleton::Skeleton()
    ...
    // TODO: Add loot items
    addLoot(new Weapon("Old Sword", "A dull, cracked blade", 3));
}
...
Dragon::Dragon()
    ...
    addLoot(new Weapon("Dragon Slayer", "Legendary sword of heroes", 10));
